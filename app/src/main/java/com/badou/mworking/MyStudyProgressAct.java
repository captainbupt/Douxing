...
public class MyStudyProgressAct extends BaseFragmentActivity implements OnClickListener,OnRefreshListener2<ListView>{
	...
	private void updataListView(final int beginNum){
		ServiceProvider.doUpdateLocalResource2(MyStudyProgressAct.this, Train.CATEGORY_TRAIN, 0, beginNum, Constant.LIST_ITEM_NUM,"","1",
		...
		pullToRefreshListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int position,
				...
				if (NetUtils.isNetConnected(MyStudyProgressAct.this)) {
					...
					ServiceProvider.doMarkRead(MyStudyProgressAct.this, train.getRid());
				}
				//返回PDF格式
				...
				// 返回html格式	
					...
					// 获取分类名
					String title = SP.getStringSP(MyStudyProgressAct.this, SP.TRAINING, train.getTag()+"", "");
	...
	private void toPdfViewer(Train train) {
		if (!MyStudyProgressAct.this.isFinishing()) {
			...
			if (android.os.Build.VERSION.SDK_INT >= 11) {
				...
				bundle.putSerializable("train", train);
				intent.putExtra("train", bundle);
				intent.setClass(MyStudyProgressAct.this, BaseViewerActivity.class);
				startActivity(intent);
				// 设置切换动画，从右边进入，左边退出
	...
	private void toPDFAndWeb(Train train) {       
		...
		if (android.os.Build.VERSION.SDK_INT >= 11) {// pdf
			...
			String path = FileUtils.getTrainCacheDir(MyStudyProgressAct.this) + train.getRid() + ".pdf";
			...
			Intent intent = new Intent(MyStudyProgressAct.this, BaseViewerActivity.class);
			Bundle bundle = new Bundle();
			bundle.putSerializable("train", train);
			intent.putExtra("train", bundle);
		...
		public void handleMessage(Message msg) {
			...
			if (train != null) {
				if(Constant.MWKG_FORAMT_TYPE_PDF == train.getSubtype()){
					 path = FileUtils.getTrainCacheDir(MyStudyProgressAct.this) + train.getRid() + ".pdf";
				}else if(Constant.MWKG_FORAMT_TYPE_MPEG == train.getSubtype()){
					 path = FileUtils.getTrainCacheDir(MyStudyProgressAct.this) + train.getRid() + ".mp4";
				}
