...
public class TongShiQuanAdapter extends BaseAdapter {
	...
	public TongShiQuanAdapter(Context mContext) {
		super();
	 ...
	 * @return
	 */
	private View getAllView(final int position, View convertView, ViewGroup parent) {
		...
		holder.imgContentPic.setOnClickListener(new ImageClickListener(
				question));
		holder.headImg.setOnClickListener(new ImageClickListener(question));
		/***评论弹出dialog监听**/
		holder.llComment.setOnClickListener(new CommentOnClick(position, question));
		/** 设置点赞数和监听 **/
	...
	static class AllViewHolder {
		...
		public AllViewHolder(View view) {
			...
			tongshiquanShipingImg = (ImageView) view.findViewById(R.id.tongshiquan_shiping_img);
						...
						new VolleyListener(mContext) {
							...
							public void onResponse(Object responseObject) {
								...
								try {
									...
									if (Net.SUCCESS != code) {
										ToastUtil
												.showToast(mContext, R.string.credit_fail);
										return;
									}
								} catch (Exception e) {
									e.printStackTrace();
	...
	class CommentOnClick implements OnClickListener{
		...
		public void onClick(View arg0) {
			...
			((Activity)mContext).startActivityForResult(intent,TongSHQFragments.requestCode);
	...
	class ShowAllContent implements OnClickListener{
		...
		public void onClick(View v) {
			Intent intent = new Intent(mContext,AroundDetailActivity.class);
			intent.putExtra(
					AroundDetailActivity.VALUE_QUESTION,
					question);
