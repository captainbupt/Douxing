 ...
 *         <p/>
 *         To fully understand this class you must know its principles : - The
 *         PDF document is seen as if we always want to draw all the pages. -
 *         The thing is that we only draw the visible parts. - All parts are the
 *         same size, this is because we can't interrupt a native page
 *         rendering, so we need these renderings to be as fast as possible, and
 *         be able to interrupt them as soon as we can. - The parts are loaded
 *         when the current offset or the current zoom level changes
 *         <p/>
 *         Important : - DocumentPage = A page of the PDF document. - UserPage =
 *         A page as defined by the user. By default, they're the same. But the
 *         user can change the pages order using
 *         {@link #load(android.net.Uri, com.joanzapata.pdfview.listener.OnLoadCompleteListener, int[])}. In this particular
 *         case, a userPage of 5 can refer to a documentPage of 17.
 */
public class PDFView extends SurfaceView {
    private int mOrientation = LinearLayout.VERTICAL;

    // 自定义监听器，然后在moveto中调用scoll该方法，在pdfview中实现该接口
    public OnScollListener onScollListener;

    /**
     * @return the onScollListener
     */
    public OnScollListener getOnScollListener() {
        return onScollListener;
    }
    ...
    public PDFView(Context context, AttributeSet attr, int defStyle) {
    ...
    /**
     * The same pages but with a filter to avoid repetition (ex: 0, 1, 1, 2, 2,
     * 3, 3, 3)
     */
    ...
    /**
     * If you picture all the pages side by side in their optimal width, and
     * taking into account the zoom level, the current offset is the position of
     * the left border of the screen in this big picture
     */
    ...
    /**
     * Async task always playing in the background and proceeding rendering
     * tasks
     */
    ...
    @SuppressLint("NewApi")
    private void load(Uri uri, OnLoadCompleteListener onLoadCompleteListener,
                      int[] userPages) {
        ...
        if (userPages != null) {
            ...
            this.filteredUserPages = ArrayUtils
                    .deleteDuplicatedPages(originalUserPages);
            this.filteredUserPageIndexes = ArrayUtils
                    .calculateIndexesInDuplicateArray(originalUserPages);
        }
    ...
    void showPage(int pageNb) {
        ...
        if (mOrientation == LinearLayout.HORIZONTAL) {
            animationManager.startXAnimation(currentXOffset,
                    calculateCenterOffsetForPage(pageNb));
        } else {
            animationManager.startYAnimation(currentYOffset,
                    calculateCenterOffsetForPage(pageNb));
        }
        loadPages();
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        loadPages();
        if (mOrientation == LinearLayout.HORIZONTAL) {
            moveTo(calculateCenterOffsetForPage(currentFilteredPage),
                    currentYOffset);
        } else {
    ...
    protected void onDraw(Canvas canvas) {
        // As I said in this class javadoc, we can think of this canvas as a
        // huge
        // strip on which we draw all the images. We actually only draw the
        // rendered
        // parts, of course, but we render them in the place they belong in this
        // huge
        // strip.
        ...
        // we can draw it but we won't see it because the canvas is not big
        // enough.

        // But if we call translate(-X, -Y) on the canvas just before drawing
        // the object :
        // _______________________________________________
        // | _____________|
        // | The big strip | |
        ...
        if (onDrawListener != null) {
            canvas.translate(toCurrentScale(currentFilteredPage
                    * optimalPageWidth), 0);

            onDrawListener.onLayerDrawn(canvas, //
                    toCurrentScale(optimalPageWidth), //
                    toCurrentScale(optimalPageHeight), currentPage);
    ...
    private void drawPart(Canvas canvas, PagePart part) {
        ...
        // TODO 设置位置 Move to the target page
        float localTranslation = 0;
        float localTranslationY = 0;

        if (mOrientation == LinearLayout.HORIZONTAL) {
            localTranslation = toCurrentScale(part.getUserPage()
                    * optimalPageWidth);
            canvas.translate(localTranslation, 0);
        } else {
            localTranslationY = toCurrentScale(part.getUserPage()
                    * optimalPageHeight);
            canvas.translate(0, localTranslationY);
        }
        ...
        float offsetX = toCurrentScale(pageRelativeBounds.left
                * optimalPageWidth);
        float offsetY = toCurrentScale(pageRelativeBounds.top
                * optimalPageHeight);
        float width = toCurrentScale(pageRelativeBounds.width()
                * optimalPageWidth);
        float height = toCurrentScale(pageRelativeBounds.height()
                * optimalPageHeight);
        ...
        if (Constants.DEBUG_MODE) {
            debugPaint.setColor(part.getUserPage() % 2 == 0 ? Color.RED
                    : Color.BLUE);
    ...
    /**
     * Load all the parts around the center of the screen, taking into account X
     * and Y offsets, zoom level, and the current page displayed
     */
    ...
    /**
     * Render a page, creating 1 to <i>nbOfPartsLoadable</i> page parts. <br>
     * <br>
     * <p/>
     * This is one of the trickiest method of this library. It finds the
     * DocumentPage associated with the given UserPage, loads its thumbnail, cut
     * this page into 256x256 blocs considering the current zoom level,
     * calculate the bloc containing the center of the screen, and start loading
     * these parts in a spiral {@link SpiralLoopManager}, only if the given part
     * is not already in the Cache, in which case it moves the part up in the
     * cache.
     *
     * @param userPage
     *            The user page to load.
     * @param nbOfPartsLoadable
     *            Maximum number of parts it can load.
    ...
    private int loadPage(final int userPage, final int nbOfPartsLoadable) {
        ...
        final float pageRelativePartWidth = 1f / nbCols;
        final float pageRelativePartHeight = 1f / nbRows;
        ...
        float middleOfScreenPageX = middleOfScreenX - userPage
                * toCurrentScale(optimalPageWidth);
        float middleOfScreenPageY = middleOfScreenY;
        float middleOfScreenPageXRatio = middleOfScreenPageX
                / toCurrentScale(optimalPageWidth);
        float middleOfScreenPageYRatio = middleOfScreenPageY
        ...
        class SpiralLoopListenerImpl implements
                SpiralLoopManager.SpiralLoopListener {
            int nbItemTreated = 0;
            ...
            public boolean onLoop(int row, int col) {
                ...
                RectF pageRelativeBounds = new RectF(relX, relY, relX
                        + relWidth, relY + relHeight);

                if (renderWidth != 0 && renderHeight != 0) {

                    // Check it the calculated part is already contained in the
                    // Cache
                    // If it is, this call will insure the part will go to the
                    // right
                    // place in the cache and won't be deleted if the cache need
                    // space.
                    if (!cacheManager.upPartIfContained(
                            userPage,
                            documentPageFinal, //
                            renderWidth, renderHeight, pageRelativeBounds,
                            nbItemTreated)) {

                        // If not already in cache, register the rendering
                        // task for further execution.
                        renderingAsyncTask.addRenderingTask(
                                userPage,
                                documentPageFinal, //
                                renderWidth, renderHeight, pageRelativeBounds,
                                false, nbItemTreated);
    ...
    public void loadComplete(DecodeService decodeService) {
        try {
            ...
            if (mOnOpenPDFFailedListener != null)
                mOnOpenPDFFailedListener.onOpenFailed();
            e.printStackTrace();
            return;
        }
    ...
    public void setOnOpenPDFFailedListener(OnOpenPDFFailedListener onOpenPDFFailedListener) {
        this.mOnOpenPDFFailedListener = onOpenPDFFailedListener;
    }

    private OnOpenPDFFailedListener mOnOpenPDFFailedListener;

    public interface OnOpenPDFFailedListener {
        void onOpenFailed();
    }

    /**
     * Called when a rendering task is over and a PagePart has been freshly
     * created.
     *
     * @param part
    ...
    /**
     * Given the UserPage number, this method restrict it to be sure it's an
     * existing page. It takes care of using the user defined pages if any.
     *
     * @param userPage
     *            A page number.
    ...
    /**
     * Calculate the x-offset needed to have the given page centered on the
     * screen. It doesn't take into account the zoom level.
     *
     * @param pageNb
     *            The page number.
    ...
    private float calculateCenterOffsetForPage(int pageNb) {
        float imageX = 0;
        if (mOrientation == LinearLayout.HORIZONTAL) {
            imageX = -(pageNb * optimalPageWidth);
            imageX += getWidth() / 2 - optimalPageWidth / 2;
        } else {
            imageX = -(pageNb * optimalPageHeight);
            imageX += getHeight() / 2 - optimalPageHeight / 2;
        }
    ...
    /**
     * Calculate the optimal width and height of a page considering the area
     * width and height
     */
    ...
    /**
     * Place the minimap background considering the optimal width and height and
     * the MINIMAP_MAX_SIZE.
     */
    private void calculateMinimapBounds() {
        ...
        minimapBounds = new RectF(getWidth() - 5 - minimapWidth, 5,
                getWidth() - 5, 5 + minimapHeight);
        calculateMinimapAreaBounds();
    }

    /**
     * Place the minimap current rectangle considering the minimap bounds the
     * zoom level, and the current X/Y offsets
     */
    private void calculateMinimapAreaBounds() {
        ...
        if (zoom == 1f) {
            ...
            float x = (-currentXOffset - toCurrentScale(currentFilteredPage
                    * optimalPageWidth)) //
                    / toCurrentScale(optimalPageWidth) * minimapBounds.width();
            float width = getWidth() / toCurrentScale(optimalPageWidth)
                    * minimapBounds.width();
            float y = -currentYOffset / toCurrentScale(optimalPageHeight)
                    * minimapBounds.height();
            float height = getHeight() / toCurrentScale(optimalPageHeight)
                    * minimapBounds.height();
            minimapScreenBounds = new RectF(minimapBounds.left + x,
                    minimapBounds.top + y, //
                    minimapBounds.left + x + width, minimapBounds.top + y
                    + height);
    ...
    private void calculateMasksBounds() {
        leftMask = new RectF(0, 0, getWidth() / 2
                - toCurrentScale(optimalPageWidth) / 2, getHeight());
        rightMask = new RectF(getWidth() / 2 + toCurrentScale(optimalPageWidth)
                / 2, 0, getWidth(), getHeight());
    }

    /**
     * 移动的方向
     * Move to the given X and Y offsets, but check them ahead of time to be
     * sure not to go outside the the big strip.
     *
     * @param offsetX
     *            The big strip X offset to use as the left border of the
     *            screen.
     * @param offsetY
     *            The big strip Y offset to use as the right border of the
     *            screen.
     */
    public void moveTo(float offsetX, float offsetY) {
        if (onScollListener != null) {
            onScollListener.scoll(currentPage, getPageCount());
        }
        if (mOrientation == LinearLayout.HORIZONTAL) {
            // Check Y offset
            if (toCurrentScale(optimalPageHeight) < getHeight()) {
                offsetY = getHeight() / 2 - toCurrentScale(optimalPageHeight)
                        / 2;
            } else {
                if (offsetY > 0) {
                    offsetY = 0;
            ...
            if (isZooming()) {
                if (toCurrentScale(optimalPageWidth) < getWidth()) {
                    ...
                    offsetX = getWidth()
                            / 2
                            - toCurrentScale((currentFilteredPage + 0.5f)
                            * optimalPageWidth);
                } else {
                    miniMapRequired = true;
                    if (offsetX
                        ...
                        offsetX = -toCurrentScale(currentFilteredPage
                            ...
                            + toCurrentScale((currentFilteredPage + 1)
                            * optimalPageWidth) < getWidth()) {
                        offsetX = getWidth()
                                - toCurrentScale((currentFilteredPage + 1)
                                * optimalPageWidth);
                    }
            ...
            // Check Y offset
            if (toCurrentScale(optimalPageWidth) < getWidth()) {
                offsetX = getWidth() / 2 - toCurrentScale(optimalPageWidth)
                        / 2;
            } else {
                if (offsetX > 0) {
                    offsetX = 0;
            ...
            if (isZooming()) {
                if (toCurrentScale(optimalPageHeight) < getHeight()) {
                    ...
                    if (offsetY
                            + toCurrentScale(currentFilteredPage
                            * optimalPageHeight) > 0) {
                        offsetY = -toCurrentScale(currentFilteredPage
                                * optimalPageHeight);
                    } else if (offsetY
                            + toCurrentScale((currentFilteredPage + 1)
                            * optimalPageHeight) < getHeight()) {
                        offsetY = getHeight()
                                - toCurrentScale((currentFilteredPage + 1)
                                * optimalPageHeight);
                    }
     ...
     *
     * @param dx
     *            The X difference you want to apply.
     * @param dy
     *            The Y difference you want to apply.
    ...
    /**
     * Change the zoom level, relatively to a pivot point. It will call moveTo()
     * to make sure the given point stays in the middle of the screen.
     *
     * @param zoom
     *            The zoom level.
     * @param pivot
    ...
    /** @see #zoomCenteredTo(float, android.graphics.PointF) */
    ...
    public class Configurator {
        ...
        public Configurator onPageChange(
                OnPageChangeListener onPageChangeListener) {
            this.onPageChangeListener = onPageChangeListener;
            return this;
        }
