...
public class HXNotifier {
    ...
    public synchronized void onNewMsg(EMMessage message) {
        ...
        List<String> blockList = EMChatManager.getInstance().getChatOptions().getGroupsOfNotificationDisabled();
        if (blockList != null && blockList.contains(message.getTo())) {
            return;
        }
        // 判断app是否在后台
    ...
    public synchronized void onNewMesg(List<EMMessage> messages) {
        ...
        List<String> blockList = EMChatManager.getInstance().getChatOptions().getGroupsOfNotificationDisabled();
        if (blockList != null && blockList.contains(messages.get(messages.size() - 1).getTo())) {
            return;
        }
        // 判断app是否在后台
        if (!EasyUtils.isAppRunningForeground(appContext)) {
